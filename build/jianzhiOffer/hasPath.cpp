/*
 * @Author: Zhang Zhaoru
 * @Since: 2020-01-27 09:52:24
 * @LastTime: 2020-03-23 19:51:14
 * @LastAuthor: Zhang Zhaoru
 * @FilePath: \vscode_git\build\jianzhiOffer\hasPath.cpp
 * @Description: 
 */
...
bool isPath(const char *matrix, vector<char> flags, const char *str, int x,
...
bool hasPath(const char *matrix, int rows, int cols, const char *str) {
    ...
    for (int i = 0; i < rows; i++)
        for (int j = 0; j < cols; j++) {
            condition =
                (condition || isPath(matrix, flags, str, i, j, rows, cols));
